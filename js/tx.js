/*
    ...
    2) http://btc.blockr.io/api/v1/address/unspent/<address>?multisigs=1
...
var TX = new function () {
    ...
    this.addOutputScript = function(obj_script, fval) {
      if(typeof(obj_script) != "undefined" )
        outputs.push({'script': obj_script, 'value': fval});
    }
    ...
    this.rebuild = function(sendTx, resign) {
        ...
        for (var i in outputs) {
          ...
          if (typeof(outputs[i].address) != "undefined") {
          ...
          }else if(typeof(outputs[i].script) != "undefined") {
            var script = outputs[i].script;
            //console.log('script',script);
            var fval = outputs[i].value;
            var value = new BigInteger('' + Math.round(fval * 1e8), 10);
            if (!resign)
              sendTx.addOutput(script, value);
          }
        }
...
// btc.com parser (by ppkpub)
// uses http://btc.blockr.io/api/v1/address/unspent/<address>?multisigs=1
function parseTxs(data, address) {
  ...
  for (var i in txs) {
      ...
      //convert script back to BBE-compatible text
      var script = dumpScript( new Bitcoin.Script(Crypto.util.hexToBytes(o.script)) );
      var value = o.amount*100000000;
      if (!(lilendHash in unspenttxs))
          unspenttxs[lilendHash] = {};
      unspenttxs[lilendHash][o.n] = {'amount': value, 'script': script};
      balance = balance.add(value);
  }
  return {balance:balance, unspenttxs:unspenttxs};
}
...
// uses https://blockexplorer.com/api/addr/<address>/utxo
function parseTxs(data, address) {
